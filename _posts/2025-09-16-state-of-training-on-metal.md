...
- I'm not sure what's going on. Perhaps the auto-generated MPS kernels are less efficient than the hand-tuned ones, or maybe there is a more fundamental issue such as silently falling back to the CPU. Either way, it seems that `torch.compile` on MPS isn't quite ready for prime time just yet.
