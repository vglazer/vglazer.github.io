...
- I'm not sure what's going on, but we might be silently falling back to the CPU due to missing kernels, or perhaps the auto-generated kernels are less efficient than the hand-tuned ones. Either way, it seems that `torch.compile` on MPS isn't quite ready for prime time just yet.
