...
- Apple has recently released its own ML framework, called [MLX](https://github.com/ml-explore/mlx), which supports not only MPS but also [Neural Engine (ANE)](https://en.wikipedia.org/wiki/Neural_Engine). Allgedely, anyway &mdash; [asitop](https://github.com/tlkh/asitop) didn't pick up any ANE usage during my MLX runs, FWIW.
